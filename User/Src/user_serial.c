#include "user_serial.h"

serial_t serial;

const uint8_t crc8_table[256] =
    {
        };

#define CALA_CRC(CRC, DATA) \
    CRC = CRC ^ DATA;       \
    CRC = crc8_table[CRC]

void usartIRQCB(uint8_t data)
{
    switch (serial.state)
    {
    case kSerial_IDEL:
        serial.state = ((data == 0xAA) && (serial.available == 0)) ? kSerial_HA : kSerial_IDEL;
        break;
    case kSerial_HA:
        serial.state = (data == 0x55) ? kSerial_DIR : kSerial_IDEL;
        break;
    case kSerial_DIR:
        serial.revice_pack.crc = 0;
        serial.revice_pack.id = data;
        CALA_CRC(serial.revice_pack.crc, data);
        serial.state = kSerial_CMD;
        break;
    case kSerial_CMD:
        serial.revice_pack.cmd = data;
        CALA_CRC(serial.revice_pack.crc, data);
        serial.state = kSerial_Lengt;
        break;
    case kSerial_Lengt:
        serial.revice_pack.length = data;
        CALA_CRC(serial.revice_pack.crc, data);
        serial.data_cnt = 0;
        serial.state = (serial.revice_pack.length == 0) ? kSerial_CRC : kSerial_DATA;
        break;
    case kSerial_DATA:
        serial.revice_pack.data[serial.data_cnt++] = data;
        CALA_CRC(serial.revice_pack.crc, data);
        if (serial.data_cnt >= serial.revice_pack.length)
        {
            serial.data_cnt = 0;
            serial.state = kSerial_CRC;
        }
        break;
    case kSerial_CRC:
        serial.revice_pack.crc = data;
        serial.state = kSerial_End;
        break;
    case kSerial_End:
        serial.available = ( data == 0xee ) ? 1 : 0;
        serial.state = kSerial_IDEL;
        break;
    default:
        serial.state = kSerial_IDEL;
        break;
    }
}


int16_t _isAvailable(void)
{
    return (serial.available == 1) ? serial.revice_pack.length : -1;
}

void clearReadReadyFlag()
{
    serial.available = 0;
}

int _revicePack(transmission_pack_t *pack_ptr)
{
    if (pack_ptr == NULL)
        return -1;
    if (serial.available != 1)
        return -1;
    
    memcpy((void *)pack_ptr, (void *)&serial.revice_pack, sizeof(transmission_pack_t));
    if( serial.revice_pack.length > 0 )
        memcpy((void *)pack_ptr->data, (void *)&serial.revice_pack.data, (size_t)serial.revice_pack.length);

    memset((void *)serial.revice_pack.data,0,sizeof(uint8_t)*256);
    return 0;
}

void _sendPack(uint8_t id,uint8_t cmd, uint8_t length, void *data)
{
    while (LL_USART_IsActiveFlag_TC(USART2) == 0UL) LL_mDelay(1);
    LL_DMA_ClearFlag_TC2(DMA1);

    uint8_t crc = 0;
    uint8_t *data_ptr = (uint8_t *)data;
    serial.tx_buff[0] = 0xAA;   
    serial.tx_buff[1] = 0x55;
    serial.tx_buff[2] = id;
    serial.tx_buff[3] = cmd;
    serial.tx_buff[4] = length;

    for (int i = 0; i < 5; i++)
    {
        crc = CALA_CRC(crc, serial.tx_buff[i]);
    }

    for (int i = 0; i < length; i++)
    {
        serial.tx_buff[5+i] = data_ptr[i];
        crc = CALA_CRC(crc,serial.tx_buff[5+i]);
    }
    serial.tx_buff[5+length + 0] = crc;
    serial.tx_buff[5+length + 1] = 0xee;

    LL_DMA_DisableChannel(DMA1,LL_DMA_CHANNEL_2);
	LL_DMA_SetDataLength(DMA1,LL_DMA_CHANNEL_2,7+length);
	LL_DMA_EnableChannel(DMA1, LL_DMA_CHANNEL_2);
}

void _serial_printf(const char *format, ...)
{
	char str_buff[256];
	va_list ap;
	va_start(ap,format);
	int length = vsprintf(str_buff,format,ap);
	serial.sendPack(0xff,0x30,length,(void*)str_buff);
	va_end(ap);
}

void _serial_log(int type,const char *format, ...)
{
	char str_buff[256];
	va_list ap;
	va_start(ap,format);
	int length = vsprintf(str_buff,format,ap);
	serial.sendPack(0xff,0x31+type,length,(void*)str_buff);
	va_end(ap);
}

void InitTransmission(USART_TypeDef *huart)
{
    serial.state = kSerial_IDEL;
    serial.huart = huart;
    serial.data_length = 0;
    serial.data_cnt = 0;
    serial.available = 0;

    serial.isAvailable = &_isAvailable;
    serial.clearRDFlag = &clearReadReadyFlag;

    serial.revicePack = &_revicePack;
    serial.sendPack = &_sendPack;
    serial.printf = &_serial_printf;
    serial.log = &_serial_log;
    serial.revice_pack.data = serial.rx_buff;

    LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_2,(uint32_t)(serial.tx_buff));
	LL_DMA_SetPeriphAddress(DMA1, LL_DMA_CHANNEL_2,LL_USART_DMA_GetRegAddr(serial.huart,LL_USART_DMA_REG_DATA_TRANSMIT));
	LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_2, 0);
	LL_USART_EnableDMAReq_TX(serial.huart);

    LL_USART_EnableIT_RXNE_RXFNE(serial.huart);
}
